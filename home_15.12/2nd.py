...
def main_shlyapa(list_X):
    ...
    range_to[0], range_to[1] = (
        round(float(range_to[0]), 1), round(float(range_to[1]), 1))
    result_list1 = set()
    result_list2 = set()
    result_list3 = set()
    result_list4 = set()
    while len(result_list1) < count_rockets:
        result_list1.add(str(random_g(words[0], words[1])).replace(
            ']', '').replace("'", "").replace('[', '') + str(randint(10, 99)))
    while len(result_list2) < count_rockets:
        result_list2.add(str(random_g(weight[0], weight[1])).replace(
            ']', '').replace('', "").replace('[', ''))
    while len(result_list3) < count_rockets:
        result_list3.add(str(random_g(range_to[0], range_to[1])).replace(
            ']', '').replace('', "").replace('[', ''))
...
print_rocket(main_shlyapa(list_of_weight))
